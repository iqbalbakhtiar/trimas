...
<div class="toolbar">
	...
	<c:if test='${access.edit}'>
		<a class="item-button-save save-person" ><span><spring:message code="sirius.save"/></span></a>
	</c:if>
</div>
<div class="main-box">
	<sesform:form id="editForm" name="editForm" method="post" modelAttribute="person_edit" enctype="multipart/form-data">
		<table style="border:none" width="100%">
			<tr>
				<td width="24%" align="right"><spring:message code="salesperson.code"/> :</td>
				<td width="70%">
					<input class='input-disabled' value="${person_edit.code}" disabled/>
			...
			<tr>
				...
				<td><input id="birthDate" name="birthDate" value="<fmt:formatDate value='${person_edit.birthDate}' pattern='dd-MM-yyyy'/>" class="datepicker"/></td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td>
					...
					<c:if test='${person_edit.picture != null}'>
						<a href="javascript:openpopup('<c:url value='/page/partyshowimage.htm?picture=${person_edit.picture}'/>');"><spring:message code="sirius.show"/></a>
					</c:if>
				</td>
			</tr>
			<tr>
		...
		<div id="mainTab" dojoType="TabContainer" style="width:100% ; height: 250px;">
			<div id="address" dojoType="ContentPane" label='<spring:message code="postaladdress"/>' class="tab-pages">
				<div class="toolbar-clean">
					<a class="item-button-new" href="<c:url value='/page/postaladdresspreadd.htm?party=${person_edit.id}&relationshipId=${relationship.id}&uri=salespersonpreedit.htm'/>"><span><spring:message code="postaladdress.new"/></span></a>
				...
				<table class="table-list" cellspacing="0" cellpadding="0" width="100%">
					...
					<c:forEach items="${person_edit.postalAddresses}" var="postal">
						<tr>
							<td class="tools">
			...
			<div id="contact" dojoType="ContentPane" label='<spring:message code="contactmechanism"/>' class="tab-pages" refreshOnShow="true">
				<div class="toolbar-clean">
					<a class="item-button-new" href="<c:url value='/page/contactmechanismpreadd.htm?party=${person_edit.id}&relationshipId=${relationship.id}&uri=salespersonpreedit.htm'/>"><span><spring:message code="contactmechanism.new"/></span></a>
					<div class="item-navigator">&nbsp;</div>
				</div>
				<table class="table-list" cellspacing="0" cellpadding="0" width="100%">
					<tr>
					...
					<c:forEach items="${person_edit.contactMechanisms}" var="contact">
						<tr>
							<td class="tools">
...
<div class="info"><spring:message code="sirius.createdby"/> : <c:out value='${person_edit.createdBy.fullName}'/> (<fmt:formatDate value='${person_edit.createdDate}' pattern='dd-MM-yyyy HH:mm:ss'/>) | <spring:message code="sirius.updatedby"/> : <c:out value='${person_edit.updatedBy.fullName}'/> (<fmt:formatDate value='${person_edit.updatedDate}' pattern='dd-MM-yyyy HH:mm:ss'/>)</div>
...
<script type="text/javascript">
	$(function(){
		$('.save-person').click(function()
		{
			...
			xhr.open('POST', "<c:url value='/page/salespersonedit.htm'/>");
