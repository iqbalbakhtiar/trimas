			...
			<td>
				<select id="referenceType" name='referenceType'>
					...
					<c:forEach items="${referenceTypes}" var="refType">
						<option value='${refType}' ${not empty filterCriteria.referenceType and filterCriteria.referenceType eq refType ? "selected" : ""}><spring:message code="receiptreferencetype.${refType.messageName}"/></option>
					</c:forEach>
				</select>
			</td>
		</tr>
...
<script type="text/javascript">
	function searchs()
	{
		var url = "<c:url value='/page/receiptmanualtypeview.htm'/>";
	...
	function resets()
	{
		window.location = "<c:url value='/page/receiptmanualtypeview.htm'/>";
	}
	...
	function step(action)
	{
		...
		document.filterForm.action = "<c:url value='/page/receiptmanualtypeview.htm?page='/>"+page;
		document.filterForm.submit();
	}
</script>	