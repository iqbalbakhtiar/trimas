		...
		<tr>
			...
			<td>
				<select id="paymentMethodType" name="paymentMethodType" class="combobox">
					...
					<option value="CASH" ${filterCriteria.paymentMethodType eq 'CASH' ? 'selected' : ''}>CASH</option>
					<option value="TRANSFER" ${filterCriteria.paymentMethodType eq 'TRANSFER' ? 'selected' : ''}>TRANSFER</option>
				</select>
			</td>
		</tr>
		<tr>
...
<script type="text/javascript">
	function searchs()
	{
		var url = "<c:url value='/page/paymentmanualview.htm'/>";
	...
	function step(action)
	{
		...
		document.filterForm.action = "<c:url value='/page/paymentmanualview.htm?page='/>"+page;
		document.filterForm.submit();
	}
</script>	