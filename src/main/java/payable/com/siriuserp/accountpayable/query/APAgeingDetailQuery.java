...
public class APAgeingDetailQuery extends AbstractStandardReportQuery {
    @Override
    public Object execute()
    {
        ...
        for(Payable verification : getPayables())
        {
            ...
            for (PaymentApplication application : verification.getApplications())
                // Original code uset getRealDate instead of .getDate
...
//                    unpaidAmount = unpaidAmount.subtract(memo.getAmountAfterTax());
            ...
            if (unpaidAmount.abs().compareTo(BigDecimal.ONE) > 0)
            {
                ...
                APAgeingDetailAdapter detail = new APAgeingDetailAdapter();
                detail.setInvoice(verification);

                detail.setBalance(detail.getBalance().add(unpaidAmount));

                if (days <= 0)
                {
                    adapter.setNotYetDue(adapter.getNotYetDue().add(unpaidAmount));
                    detail.setNotYetDue(detail.getNotYetDue().add(unpaidAmount));
                } else if (days <= 30)
                {
                    adapter.setFstOverDue(adapter.getFstOverDue().add(unpaidAmount));
                    detail.setFstOverDue(detail.getFstOverDue().add(unpaidAmount));
                } else if (days <= 60)
                {
                    adapter.setSndOverDue(adapter.getSndOverDue().add(unpaidAmount));
                    detail.setSndOverDue(detail.getSndOverDue().add(unpaidAmount));
                } else if (days <= 90)
                {
                    adapter.setThdOverDue(adapter.getThdOverDue().add(unpaidAmount));
                    detail.setThdOverDue(detail.getThdOverDue().add(unpaidAmount));
                } else
                {
                    adapter.setFthOverDue(adapter.getFthOverDue().add(unpaidAmount));
                    detail.setFthOverDue(detail.getFthOverDue().add(unpaidAmount));
                }

                adapter.getItems().add(detail);
            }
    ...
    private List<Payable> getPayables()
    {
        ...
        if(SiriusValidator.validateLongParam(criteria.getCurrency()))
            builder.append("AND ver.money.currency.id =:currency ");
