...
public class APLedgerSummaryQuery extends AbstractStandardReportQuery
{
	...
	{
		StringBuilder builder = new StringBuilder();
		builder.append("SELECT SUM(payment.paymentInformation.amount) FROM Payment payment ");
		builder.append("WHERE (payment.date BETWEEN :start AND :end) ");
		builder.append("AND payment.organization.id in(:orgs) ");
		builder.append("AND payment.supplier.id =:supplier");
	...
	{
		StringBuilder builder = new StringBuilder();
		builder.append("SELECT DISTINCT relation.party FROM PartyRelationship relation ");
		builder.append("WHERE relation.relationshipType.id =:relationshipType ");
		builder.append("AND relation..party.id in(:orgs) ");
		builder.append("AND relation.partyRoleType.id =:from ");
		builder.append("AND relation.partyRoleType.id =:to");
