...
public class SalesPersonService
{
	...
	public FastMap<String, Object> view(GridViewFilterCriteria filterCriteria, Class<? extends GridViewQuery> queryclass) throws Exception
	{
		...
		map.put("persons", FilterAndPaging.filter(genericDao, QueryFactory.create(filterCriteria, queryclass)));

		return map;
	}

	@Transactional(readOnly = true, propagation = Propagation.NOT_SUPPORTED)
	@InjectParty(keyName = "person_add")
	public FastMap<String, Object> preadd()
	{
		FastMap<String, Object> map = new FastMap<String, Object>();
		map.put("person_add", new Party());

		return map;
	}

	@AuditTrails(className = Party.class, actionType = AuditTrailsActionType.CREATE)
	public FastMap<String, Object> add(Party person, Long personId, MultipartFile file) throws Exception
	{
		Long relationshipId = null;

		if (SiriusValidator.validateParamWithZeroPosibility(personId))
		{
		...
		} else
		{
			...
			relationship.setPartyRoleTypeFrom(genericDao.load(PartyRoleType.class, PartyRoleType.SALES_PERSON));
	...
	public FastMap<String, Object> preedit(Long id) throws Exception
	{
		...
		FastMap<String, Object> map = new FastMap<String, Object>();
		map.put("person_edit", load(partyRelationship.getPartyFrom().getId()));
