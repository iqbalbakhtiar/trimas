...
public class TrialBalanceReportQuery extends AbstractAccountingReportQuery
{
    public TrialBalanceReportQuery(SimpleAccountingReportAdapter simpleAccountingReportAdapter)
    {
    ...
    //rewrite code
    private List<TrialBalanceReportAdapter> trial(FastList<Long> openings, FastList<Long> periods, FastList<Long> orgs)
    {
        StringBuilder sql = new StringBuilder("SELECT new com.siriuserp.reporting.accounting.adapter.TrialBalanceReportAdapter(detail.account, " +
        		...
        		"SUM( CASE WHEN detail.postingType = :debet AND detail.journalEntry.accountingPeriod.id in(:perds) AND detail.journalEntry.entryType != 'ADJUSTMENT' AND detail.journalEntry.entrySourceType != 'CLOSING' AND detail.journalEntry.entrySourceType != 'OPENING' THEN (detail.amount) ELSE 0 END ), SUM( CASE WHEN detail.postingType = :credit AND detail.journalEntry.accountingPeriod.id in(:perds) AND detail.journalEntry.entryType != 'ADJUSTMENT' AND detail.journalEntry.entrySourceType != 'CLOSING' AND detail.journalEntry.entrySourceType != 'OPENING' THEN (detail.amount) ELSE 0 END ), " +
                "SUM( CASE WHEN detail.postingType = :debet AND detail.journalEntry.accountingPeriod.id in(:perds) AND detail.journalEntry.entryType = 'ADJUSTMENT' AND detail.journalEntry.entrySourceType != 'CLOSING' AND detail.journalEntry.entrySourceType != 'OPENING' THEN (detail.amount) ELSE 0 END ), SUM( CASE WHEN detail.postingType = :credit AND detail.journalEntry.accountingPeriod.id in(:perds) AND detail.journalEntry.entryType = 'ADJUSTMENT' AND detail.journalEntry.entrySourceType != 'CLOSING' AND detail.journalEntry.entrySourceType != 'OPENING' THEN (detail.amount) ELSE 0 END )) " +
        	"FROM JournalEntryDetail detail WHERE detail.journalEntry.entryStatus = 'POSTED' ");
