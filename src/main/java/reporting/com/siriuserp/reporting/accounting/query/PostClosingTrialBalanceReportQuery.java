...
public class PostClosingTrialBalanceReportQuery extends AbstractAccountingReportQuery
{
    public PostClosingTrialBalanceReportQuery(SimpleAccountingReportAdapter simpleAccountingReportAdapter)
    {
    ...
    private List<GLRegisterReportAdapter> trial(FastList<Long> openings, FastList<Long> periods, FastList<Long> orgs)
    {
        StringBuilder sql = new StringBuilder("SELECT new com.siriuserp.reporting.accounting.adapter.TrialBalanceReportAdapter(detail.account, " +
        		"SUM( CASE WHEN detail.postingType = :debet AND detail.journalEntry.accountingPeriod.id in(:opens) AND detail.journalEntry.entrySourceType = 'OPENING' THEN (detail.amount*detail.journalEntry.exchange.rate) ELSE 0 END )-SUM( CASE WHEN detail.postingType = :credit AND detail.journalEntry.accountingPeriod.id in(:opens) AND detail.journalEntry.entrySourceType = 'OPENING' THEN (detail.amount*detail.journalEntry.exchange.rate) ELSE 0 END ), " +
                "SUM( CASE WHEN detail.postingType = :debet AND detail.journalEntry.accountingPeriod.id in(:perds) AND detail.journalEntry.entrySourceType != 'OPENING' THEN (detail.amount*detail.journalEntry.exchange.rate) ELSE 0 END ), SUM( CASE WHEN detail.postingType = :credit AND detail.journalEntry.accountingPeriod.id in(:perds) AND detail.journalEntry.entrySourceType != 'OPENING' THEN (detail.amount*detail.journalEntry.exchange.rate) ELSE 0 END )) " +
        	"FROM JournalEntryDetail detail WHERE detail.journalEntry.entryStatus != 'PENDING' ");
