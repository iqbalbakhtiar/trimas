...
public class ProfitLossReportQuery extends AbstractAccountingReportQuery
{
    public List<ProfitLossReportAdapter> execute()
    {
        ...
        builder.append("SELECT new com.siriuserp.reporting.accounting.adapter.ProfitLossReportAdapter(detail.account, SUM( CASE WHEN detail.postingType = :debet THEN (detail.amount) ELSE 0 END )-SUM( CASE WHEN detail.postingType = :credit THEN (detail.amount) ELSE 0 END ))  FROM JournalEntryDetail detail "+indexJoin+" WHERE detail.journalEntry.entrySourceType != 'CLOSING' AND "+org+" detail.journalEntry.entryStatus = 'POSTED' AND detail.journalEntry.accountingPeriod.id in(:periods)");
        if(!accountingReportAdapter.getAppliedIndex().isEmpty())
        {
