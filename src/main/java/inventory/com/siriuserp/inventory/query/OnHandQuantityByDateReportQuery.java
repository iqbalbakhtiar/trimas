...
public class OnHandQuantityByDateReportQuery extends AbstractStandardReportQuery
{
	...
	public Object execute() 
	{
		...
		builder.append("SELECT new com.siriuserp.inventory.adapter.InventoryLedgerAdapter(SUM(inOut.quantity), inOut.product.name, inOut.container.name, inOut.date)");
		builder.append("FROM ProductInOutTransaction inOut WHERE inOut.quantity > 0 ");
		builder.append("AND inOut.organization.id =:org ");
		...
		builder.append("GROUP BY inOut.date, inOut.product.id, inOut.container.id ");
		builder.append("ORDER BY inOut.date, inOut.product.name, inOut.container.name ASC");
