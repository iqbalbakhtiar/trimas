/**
 * File Name  : OnHandQuantityLotViewQuery.java
...
public class OnHandQuantityLotViewQuery extends AbstractGridViewQuery
{
	@Override
	public Query getQuery(ExecutorType type)
	{
		...
		if (type.compareTo(ExecutorType.HQL) == 0)
			builder.append("SELECT NEW com.siriuserp.inventory.adapter.OnHandQuantityLotAdapter(inv.product, inv.lot, SUM(inv.onHand), SUM(inv.onTransfer), SUM(inv.reserved)) ");
