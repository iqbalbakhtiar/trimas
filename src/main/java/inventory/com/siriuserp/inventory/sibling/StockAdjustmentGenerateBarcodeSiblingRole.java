	...
	public void execute() throws Exception, NoSuchMethodException, SecurityException, IllegalArgumentException, InvocationTargetException
	{
		...
		if (stockAdjustment != null)
		{
			...
			if (!itemsWithEmptySerial.isEmpty() || !itemsWithSerial.isEmpty())
			{
				...
				barcodeGroup.setQuantity(stockAdjustment.getForm().getItems().stream().map(Item::getQuantity).collect(DecimalHelper.sum()));

				barcodeGroupService.add(barcodeGroup);
				stockAdjustment.setBarcodeGroup(barcodeGroup);
			}

			if (!itemsWithEmptySerial.isEmpty())
				processBarcode(itemsWithEmptySerial, barcodeGroup);

			if (!itemsWithSerial.isEmpty())
				processBarcode(itemsWithSerial, barcodeGroup);

			if (!itemsWithSerialExist.isEmpty())
				processBarcode(itemsWithSerialExist, null);
		}
	}

	@Transactional
	public void processBarcode(List<Item> items, BarcodeGroup barcodeGroup) throws Exception
	{
		if (barcodeGroup != null)
		{
			for (Item item : items)
			{
				if (item.getProduct() != null && item.getProduct().isSerial() && SiriusValidator.nz(item.getQuantity()))
				{
					...
					Barcode code = barcodeService.generateBarcode(item.getProduct().getId(), item.getQuantity(), item.getSerial(), barcodeGroup, isBarcodeValid);
